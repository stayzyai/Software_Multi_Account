import { ChevronDown } from "lucide-react";
import StatusBadge from "./StatusBadge";

const DataTable = ({
  title,
  filters = [],
  columns,
  data,
  badgeType = "status",
  badgeColumn,
  className = "w-full",
}) => {
  return (
    <div
      className={`flex flex-col border-2 border-gray-200 shadow-xl rounded-[14px] ${className} p-4`}
    >
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-[15px] font-bold">{title}</h1>
        {filters.length > 0 && (
          <div className="flex items-center gap-3">
            {filters.map((filter) => (
              <div key={filter} className="flex items-center gap-1">
                <p className="text-[13px] text-gray-600">{filter}</p>
                <button className="p-0.5 hover:bg-gray-100 rounded-full">
                  <ChevronDown className="h-4 w-4 text-gray-500" />
                </button>
              </div>
            ))}
          </div>
        )}
      </div>

      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="text-left text-[13px] border-b text-gray-600">
              {columns.map((column) => (
                <th
                  key={column.key}
                  className={`pb-2 ${column.width} font-bold`}
                >
                  {column.label}
                </th>
              ))}
            </tr>
          </thead>
          <tbody className="text-[13px]">
            {data.map((row, index) => (
              <tr key={index} className="hover:bg-gray-50">
                {columns.map((column) => (
                  <td key={column.key} className="py-2">
                    {column.key === badgeColumn ? (
                      <StatusBadge status={row[column.key]} type={badgeType} />
                    ) : (
                      row[column.key]
                    )}
                  </td>
                ))}
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
};

export default DataTable;
