...
import ListingGuestBooking from "./ListingGuestBookingDetails";
import { useState } from "react";

const BookingGroup = ({ dateRange, bookings, setGuestOpen, setGuestInfo }) => {
  return (
    <div className="space-y-2 text-[#000000]">
      <div className="md:text-[22px] font-semibold border-b border-gray-300 py-1 font-inter">
        {dateRange}
      </div>
      {!bookings ? (
...
const ListingBookingDetails = ({ calenderDetails }) => {
  ...
  if (!bookings?.length && !dateRanges?.length) {
    return (
      <p className="text-center text-gray-600 flex items-center text-xl w-full h-96 justify-center">
        No booking details
      </p>
    );
  }
