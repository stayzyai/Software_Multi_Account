const VARIABLES = [
  { label: "Guest name", value: "guest_name" },
  { label: "Discount", value: "discount" },
  { label: "Old check-in date", value: "old_check_in_date" },
  { label: "Old check-out date", value: "old_check_out_date" },
  { label: "New check-in date", value: "new_check_in_date" },
  { label: "New check-out date", value: "new_check_out_date" },
  { label: "Total price", value: "total_price" },
  { label: "Offer days", value: "offer_days" },
  { label: "Early check-in time", value: "early_check_in_time" },
  { label: "Late check-out time", value: "late_check_out_time" },
  { label: "Fee", value: "fee" },
];

export { VARIABLES };
