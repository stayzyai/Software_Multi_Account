...
const SearchResultsList = ({ searchResults, handleSelectResult }) => {
  ...
  const listings = searchResults?.filter((result) => !result.recipientName);
  ...
  return (
    <ul className="absolute top-full left-0 mt-2 w-full max-h-60 overflow-y-auto bg-white shadow-lg rounded-md overflow-hidden z-10">
      {listings.length > 0 && (
        <>
          <li className="px-4 py-2 bg-gray-200 font-bold text-gray-700">Listings</li>
      ...
      {conversations?.length > 0 && (
        <>
          <li className="px-4 py-2 bg-gray-200 font-bold text-gray-700">Conversations</li>
