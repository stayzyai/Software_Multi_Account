...
import { simplifiedResult, filterReservations } from "../../../../helpers/Message";
...
const getFirstTwoWords = (name)=>{
  ...
  const handleApplyFilter = ()=>{
    ...
    const matchingConversations = conversation.filter(convo => listingMapIds.includes(convo.listingMapId));
    ...
    <div className="flex max-h-screen bg-[#fff]">
      <div className={`transition-all duration-300 ${ openSidebarMessage ? "z-50 sm:z-0 fixed sm:sticky top-[70px] sm:top-0 h-full sm:h-auto left-0 bg-white": "hidden md:block"} lg:w-[230px] xl:w-[257px] bg-[#FCFDFC] border-r border-gray-300`}>
        ...
        <div className="space-y-4 overflow-y-scroll scrollbar-hide h-[calc(100vh-71px)] pb-4">
          {(filteredConversations.length > 0 ? filteredConversations : fromatedConversation).map((item, index) => (
            <div key={index} onClick={() =>{ handleClickMessages(item?.id, fromatedConversation); dispatch(markChatAsRead({chatId:item.id})) }} className={`flex items-center space-x-2 cursor-pointer mt-[14px] rounded-3xl w-full xl:px-3 px-1 h-12  ${ unreadChats[item.id] ? 'bg-green-100 hover:bg-green-100' : "bg-white hover:bg-gray-50 "}`}>
